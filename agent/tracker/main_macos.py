#!/usr/bin/env python3
"""
Activity Tracker Agent for macOS
...
from backends.macos import MacOSActivityCollector, MacOSSystemStats
...
class TrackerAgent:
    def __init__(self, config_path: str = None):
        ...
        self.system_stats = MacOSSystemStats()
        self.collector = MacOSActivityCollector()
        self._running = False
    ...
    def run(self):
        ...
        print(f"Starting Activity Tracker Agent (macOS)")
        ...
        try:
            while self._running:
                ...
                print(f"[{event['timestamp'][:19]}] keys={event['key_count']}, clicks={event['mouse_clicks']}, "
                      f"app={event['active_app']}, cpu={event['cpu_percent']}%, ram={event['ram_used_percent']}%")
